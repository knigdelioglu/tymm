...
class GenericTeacherGuideProjectionTests(unittest.TestCase):
    ...
    def test_source_change_changes_top_level_runtime_fingerprint(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            self._source(root)
            first = compiler.compiler_state(root)[1]
            section_path = root / "teacher_guide" / "force" / "experiments.json"
            section = json.loads(section_path.read_text(encoding="utf-8"))
            section["guide_units"][0]["items"][0]["teacher_guidance"] = "Yeni yönlendirme"
            section_path.write_text(
                json.dumps(section, ensure_ascii=False, indent=2) + "\n",
                encoding="utf-8",
            )
            second = compiler.compiler_state(root)[1]
