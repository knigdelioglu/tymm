...
def intersects(a: str,b: str) -> bool:
    a0,a1 = parse_range(a); b0,b1 = parse_range(b)
...
def split_answers(item: dict[str,Any]) -> list[tuple[str,Any,str|None]] | None:
    ...
    if isinstance(ans,dict) and len(ans)==n:
        return [(num,val,str(key)) for num,(key,val) in zip(nums,ans.items())]
    if isinstance(ans,list) and len(ans)==n:
        return [(num,val,None) for num,val in zip(nums,ans)]
    return None

def expected_specs(item: dict[str,Any]) -> list[dict[str,Any]]:
    split=split_answers(item); iid=item["item_id"]
    if split:
        return [{"task_id":f"{iid}#Q{n}","question_number":n,"expected_answer":a,"expected_response":None,"answer_component_key":k,"split_from_group":True} for n,a,k in split]
...
def values(block: dict[str,Any], field: str) -> list[str]:
    if field in {"support","enrichment"}:
        ...
        x=d.get(field,[]) if isinstance(d,dict) else []
    else:
        x=block.get(field,[])
    return x if isinstance(x,list) else []

def fingerprint(block: dict[str,Any]) -> tuple[Any,...]:
    return tuple(tuple(values(block,f)) for f in FIELDS)

def main() -> int:
    ap=argparse.ArgumentParser()
    ap.add_argument("--repo-root",type=Path,default=Path("."))
    ap.add_argument("--overlay",default="courses/TDE_11/teacher_guide/TEMA_01/pedagogy_v2.json")
    ap.add_argument("--manifest",default="courses/TDE_11/teacher_guide/TEMA_01/teacher_guide.json")
    ap.add_argument("--schema",default="skill/tymm-material-planner/schemas/teacher_guide_pedagogy_overlay.schema.json")
    ap.add_argument("--profiles",default="skill/tymm-material-planner/data/teacher_guide_v2_profiles.json")
    ap.add_argument("--markdown",default=None)
    a=ap.parse_args(); root=a.repo_root.resolve()
    op=root/a.overlay; mp=root/a.manifest; sp=root/a.schema
    mdp=root/a.markdown if a.markdown else op.with_name("TEACHER_GUIDE_V2.md")
    failures=[]; warnings=[]
    for p,label in ((op,"overlay"),(mp,"manifest"),(sp,"schema")):
        if not p.is_file(): failures.append(f"MISSING_{label.upper()}:{p}")
    if failures:
        print(json.dumps({"status":"FAIL","failures":failures},ensure_ascii=False,indent=2)); return 1

    overlay=read_json(op); manifest=read_json(mp); schema=read_json(sp)
    version=overlay.get("schema_version"); v21=version in {"2.1.0","2.2.0"}; v22=version=="2.2.0"
    profiles=read_json(root/a.profiles) if v22 and (root/a.profiles).is_file() else None
    markdown=mdp.read_text(encoding="utf-8") if v21 and mdp.is_file() else ""
    ...
    for err in sorted(Draft202012Validator(schema).iter_errors(overlay),key=lambda e:list(e.absolute_path)):
        loc=".".join(map(str,err.absolute_path)) or "$"; failures.append(f"SCHEMA:{loc}:{err.message}")
    ...
    # New promotion contract applies to generated v2.2+, not the manually curated legacy v2.0 Tema 1.
    ...
    refs=[]; cards_by_id={}; block_ids=set(); by_section=defaultdict(list)
    sentence_counts={f:Counter() for f in REPEAT_LIMITS}; fp_seen=defaultdict(dict)
    ...
    for block in blocks:
        ...
        if v22 and sec is not None:
            ...
            if sec.get("section_type") in {"SPEAKING","WRITING"} and block.get("phase_name")=="meaning":
                failures.append(f"PRODUCTION_SKILL_MISCLASSIFIED_AS_MEANING:{bid}")
            for field,maximum in MAX_COUNTS.items():
                vals=values(block,field)
                ...
                for text in vals:
                    if isinstance(text,str):
                        ...
                        if text in phase_templates.get(field,set()): failures.append(f"UNCONTEXTUALIZED_PHASE_TEMPLATE:{bid}:{field}:{text}")
            if values(block,"board_notes"):
                prof=((profiles or {}).get("section_profiles",{}).get(overlay.get("theme_id"),{}).get(sid,{}))
                curated=set(prof.get("board_notes",[])) if isinstance(prof,dict) else set()
                for note in values(block,"board_notes"):
    ...
    if v21:
        ...
        missing_tasks=sorted(set(specs)-set(cards_by_id)); unexpected=sorted(set(cards_by_id)-set(specs))
        if missing_tasks: failures.append("MISSING_FIRST_CLASS_TASKS:"+", ".join(missing_tasks))
        if unexpected: failures.append("UNEXPECTED_TASK_CARDS:"+", ".join(unexpected))
        unanswered=set()
        for tid,(iid,item,spec) in specs.items():
            ...
            if spec.get("answer_component_key") is not None and card.get("answer_component_key")!=spec["answer_component_key"]:
                failures.append(f"ANSWER_COMPONENT_KEY_DRIFT:{tid}")
    ...
    if v22 and isinstance(profiles,dict):
                ...
                for field in (*FIELDS,"source_limitations"):
                    ...
                    for b in sec_blocks:
                        if field=="source_limitations":
                            actual.extend(b.get(field,[]) if isinstance(b.get(field),list) else [])
                        else:
                            actual.extend(values(b,field))
                    for entry in expected:
                        if isinstance(entry,str) and entry.strip() and actual.count(entry)!=1:
                            failures.append(f"SECTION_GUIDANCE_NOT_EXACTLY_ONCE:{sid}:{field}:{actual.count(entry)}:{entry}")
        ...
        for b in blocks:
            if isinstance(b,dict) and isinstance(b.get("section_id"),str) and isinstance(b.get("pedagogical_intent"),str):
                intents[b["section_id"]][b["pedagogical_intent"]]+=1
    ...
    report={"status":status,"course_id":overlay.get("course_id"),"theme_id":overlay.get("theme_id"),"schema_version":version,"overlay_status":overlay.get("status"),"canonical_item_count":len(items),"covered_item_count":len(set(refs)&set(items)),"task_card_count":len(cards_by_id) if v21 else None,"question_task_card_count":sum(1 for x in cards_by_id.values() if x.get("item_type")=="QUESTION") if v21 else None,"block_count":len(blocks),"blocks_with_board_notes":sum(1 for b in blocks if isinstance(b,dict) and b.get("board_notes")),"semantic_quality_gates":v22,"failures":failures,"warnings":warnings}
