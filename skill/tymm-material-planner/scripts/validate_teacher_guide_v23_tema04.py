#!/usr/bin/env python3
"""Source-parity gate for Theme 4 Teacher Guide V2.3 checkpoint (s.236-262)."""
...
EXPECTED = {
    ...
    "entries": 39,
    "questions": 27,
    "recognizable_questions": 27,
    "component_projected_entries": 29,
    "shared_canonical_items": 9,
    "review_required_fragments": 0,
    "component_registry_entries": 21,
    "component_registry_used": 21,
}
...
def main() -> int:
    ...
    # Critical source parity: the source has three real Metni Anlayalım questions, not V2's four artificial projections.
    ...
    # Natural-process boundaries that must not become question banks.
    process_ids = {
    ...
    for mid in process_ids:
        row = by_id.get(mid, {})
        if row.get("presentation_type") != "PROCESS":
    ...
    # p262 had two real source questions hidden by the older canonical grouping; they must stay visible.
